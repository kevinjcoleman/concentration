...
class Card extends React.Component {
  ...
  backgroundClass(order) {
    if (order <= 5 || (order > 11 && order <= 17)) {
      return `covered ${order % 2 == 0 ? "red-covered" : "blue-covered"}`;
    } else {
      return `covered ${order % 2 == 0 ? "blue-covered" : "red-covered"}`;
    }
  }

  render() {
    //Alternate background colors of covered cards.
    var coveredClass = this.backgroundClass(this.props.order);
